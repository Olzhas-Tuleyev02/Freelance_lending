...
const PortfolioItem: React.FC<{ icon: React.ReactNode; title: string; description: string; highlight?: boolean; format?: boolean }> = ({ icon, title, description, highlight, format }) => (
    <div className={`bg-gray-800/50 p-6 rounded-xl shadow-lg border transition-all duration-300 transform hover:-translate-y-2 h-full flex flex-col ${format ? 'bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border-indigo-500 ring-1 ring-indigo-500/50' : highlight ? 'border-indigo-500/60 bg-indigo-900/10' : 'border-gray-700/50 hover:shadow-indigo-500/30'}`}>
        <div className={`mb-4 ${format ? 'text-white' : highlight ? 'text-indigo-300' : 'text-indigo-400'}`}>{icon}</div>
        <h3 className="text-xl font-bold text-white mb-2">{title}</h3>
        <p className={`${format ? 'text-gray-200' : 'text-gray-400'} flex-grow`}>{description}</p>
    </div>
);
        ...
        <section id="portfolio" className="py-20 md:py-32 bg-gray-900/50">
            <div className="container mx-auto px-6">
                <div className="text-center mb-16 animate-fade-in-up">
                    ...
                    <p className="mt-4 text-lg text-gray-400">Пакет "Все включено": Портфолио + Инструменты + Ментор.</p>
